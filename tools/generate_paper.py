...
body('图 1 展示了 7 条记忆的强度随时间变化曲线，图 2 为强度热力图，'
     '图 3 为每周分层分布的堆叠柱状图。')

try:
    doc.add_picture(str(Path(__file__).resolve().parent / 'sim_charts' / 'strength_curves.png'),
                    width=Inches(5.5))
    table_caption('图 1  30 天记忆强度演化曲线。L1/L2/L3 背景色带标注。')
    ...
    doc.add_picture(str(Path(__file__).resolve().parent / 'sim_charts' / 'layer_distribution.png'),
                    width=Inches(4.5))
    table_caption('图 3  每周分层分布（堆叠柱状图）。L1/L2/L3/COLD 占比随时间变化。')
except Exception:
